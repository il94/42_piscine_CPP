...
void	Bureaucrat::signForm(A_Form& document)
{
	try
	{
...
void	Bureaucrat::executeForm(A_Form const & document)
{
	try
	{
		document.execute(*this);
		std::cout << getName() << " executed " << document.getName() << std::endl;
	}
	catch (A_Form::FormException& e)
	{
		std::cout << getName() << " couldn't execute " << document.getName() << "." << std::endl;
		std::cout << e.what() << std::endl;
	}
}
